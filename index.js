...
Only output command lines, nothing else.`;
    ...
    function setupEventListeners() {
        ...
        window.clearChat = async function() {
            ...
            setTimeout(async () => {
                try {
                    // Update the current character first to get the new characterId and chatId
                    updateForCurrentCharacter();
                    
                    // Restore the bot outfit data, but map from old chatId to new chatId
                    // The instance part (based on first message) should remain the same
                    if (botManager && botManager.characterId && botManager.chatId) {
                        // Since the savedBotOutfits contains variables with the old chatId,
                        // we need to map them to the new chatId while keeping the instance ID part
                        for (const [oldVarName, value] of Object.entries(savedBotOutfits)) {
                            // Parse the old variable name to extract the instance ID and slot
                            // Format: OUTFIT_INST_<characterId>_<oldChatId>_<instanceId>_<slot>
                            const oldPattern = new RegExp(`^OUTFIT_INST_([^_]+)_([^_]+)_(.+)_([^_]+)// Define base directory for imports
const BASE_PATH = new URL('.', import.meta.url).pathname;
                ...
                if (context && context.chat && Array.isArray(context.chat)) {
                    ...
                    if (typeof updateChatOutput === 'function') {
                        updateChatOutput();
                            ...
                            const match = oldVarName.match(oldPattern);
                            
                            if (match) {
                                const [, charId, oldChatId, instanceId, slot] = match;
                                // Create the new variable name using the new chatId but same instance ID and slot
                                const newVarName = `OUTFIT_INST_${charId}_${botManager.chatId}_${instanceId}_${slot}`;
                                
                                // Set the value to the new variable name
                                botManager.setGlobalVariable(newVarName, value);
                            }
                        }
                        
                        if (Object.keys(savedBotOutfits).length > 0) {
                            console.log(`[OutfitTracker] Mapped ${Object.keys(savedBotOutfits).length} bot outfit instances from old chat ID to new chat ID after reset`);
                            
                            // Load the outfit data to update the manager's current values
                            botManager.loadOutfit();
                        }
                    }

                    // For user outfits, the mapping is simpler - they are instance-based, not chatId-specific
                    if (userManager) {
                        for (const [varName, value] of Object.entries(savedUserOutfits)) {
                            // User outfit variables follow pattern: OUTFIT_INST_USER_<instanceId>_<slot>
                            userManager.setGlobalVariable(varName, value);
                        }
                        
                        if (Object.keys(savedUserOutfits).length > 0) {
                            console.log(`[OutfitTracker] Restored ${Object.keys(savedUserOutfits).length} user outfit instances after chat clear`);
