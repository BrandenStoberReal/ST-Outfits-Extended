...
jest.mock('../src/utils/utilities.ts', () => ({
...
class MockOutfitStore {
    ...
    notifyListeners() {
        this.state.listeners.forEach((listener: (arg0: any) => void) => {
            try {
                listener(this.state);
            } catch (error) {
    ...
    subscribe(listener: any) {
        ...
        return () => {
            this.state.listeners = this.state.listeners.filter((l: any) => l !== listener);
        };
    }

    getBotOutfit(characterId: string, instanceId: string) {
    ...
    setBotOutfit(characterId: string, instanceId: string, outfitData: any) {
    ...
    setCurrentInstanceId(instanceId: string) {
    ...
    setPanelVisibility(panelType: string, isVisible: boolean) {
        this.state.panelVisibility[panelType] = isVisible;
        this.notifyListeners();
    }

    getPanelVisibility(panelType: string) {
        return this.state.panelVisibility[panelType];
    }
}

describe('Outfit Store', () => {
    let store: MockOutfitStore;
